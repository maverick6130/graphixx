 ...
 : mWidth(width), mHeight(height), mTitle(title), mOpenGlVersion(openGlVersion), mScene(scene)
{
    ...
    glfwSetKeyCallback(mWindowPtr, Callback::input);
}
...
void Window::HandleInput(int key, int scancode, int action, int mods)
{
    mScene->HandleInput(key, scancode, action, mods);
}
